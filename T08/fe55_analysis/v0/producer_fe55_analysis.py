...
slotdirs = siteUtils.dependency_glob('??',
                                       jobname=siteUtils.getProcessName('fe55_acq'),
                                       description='Fe55 files:')
for slotd in slotdirs :
    fe55_files = siteUtils.dependency_glob('%s/*_fe55_fe55_*.fits' & slotd,
                                       jobname=siteUtils.getProcessName('fe55_acq'),
                                       description='Fe55 files:')
    print "fe55_files ="
    print fe55_files
...
#
    bias_files = siteUtils.dependency_glob('%s/*_fe55_bias_*.fits',
                                       jobname=siteUtils.getProcessName('fe55_acq'),
                                       description='Bias files:')
    nf = len(bias_files)
#    outfile = '%(raft_id)s_mean_bias_%(nf)i.fits' % locals()
    outfile = '%s_mean_bias_%(nf)i.fits' % slotd
...
#pylab.savefig('%(raft_id)s_fe55_zoom.png' % locals())
    pylab.savefig('%s_fe55_zoom.png' % slotd)
...
#    rolloff_mask_file = '%s_rolloff_defects_mask.fits' % raft_id
    rolloff_mask_file = '%s_rolloff_defects_mask.fits' % slotd
