...
builder.main (RaftsMain, clientFactory:factory, nodeTags:taskConfig) {
    ...
    for (int i = 0; i < nreb; i++) {
        ...
        "$reb" (REBDevice, hdwType:"daq1", id:i, ifcName:"", ccdMask:7, clientFactory:factory) {
            ...
            //"${reb}.DAC" (DacControl, raw: true)   // Raw DAC values
            "${reb}.DAC" (DacControl)    // Physical values
            ...
            for (int j = 0; j < 3; j++) {
                //"${reb}.Bias$j" (BiasControl, hwChan: j, raw: true)   // Raw DAC values
                "${reb}.Bias$j" (BiasControl, hwChan: j)   // Physical values
            }
        }
        ...
        "${reb}.DigP"    (Channel, description: "Digital PS power", format: ".2f", units: "Watts",
                          devcName: "Calc", type: "PROD", subtype: "${reb}.DigV:${reb}.DigI", scale: 0.001, pageId: i)
        ...
        "${reb}.AnaP"    (Channel, description: "Analog PS power", format: ".2f", units: "Watts",
                          devcName: "Calc", type: "PROD", subtype: "${reb}.AnaV:${reb}.AnaI", scale: 0.001, pageId: i)
        ...
        "${reb}.ClkHP"   (Channel, description: "Clock High PS power", format: ".2f", units: "Watts",
                          devcName: "Calc", type: "PROD", subtype: "${reb}.ClkHV:${reb}.ClkHI", scale: 0.001, pageId: i)
        ...
        "${reb}.ClkLP"   (Channel, description: "Clock Low PS power", format: ".2f", units: "Watts",
                          devcName: "Calc", type: "PROD", subtype: "${reb}.ClkLV:${reb}.ClkLI", scale: 0.001, pageId: i)
        ...
        "${reb}.ODP"     (Channel, description: "OD PS power", format: ".2f", units: "Watts",
                          devcName: "Calc", type: "PROD", subtype: "${reb}.ODV:${reb}.ODI", scale: 0.001, pageId: i)
